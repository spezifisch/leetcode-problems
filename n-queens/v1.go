...
func (g *Grid) PlaceQueen(row, col int) {
	...
	if g.queensPlaced >= g.n {
		panic("too many queens")
	}
...
func (s *Solver) solveNQueensRecurse(grid *Grid, myRow int) {
	if myRow == s.N {
		...
		//fmt.Println("found solution", *grid)
		return
	}
...
// Runtime: 18 ms, faster than 16.53% of Go online submissions for N-Queens.
// Memory Usage: 6.2 MB, less than 12.40% of Go online submissions for N-Queens.
