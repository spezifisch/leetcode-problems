class Solution:
    def combinationSum(self, candidates: List[int], target: int, recursed: bool=False) -> List[List[int]]:
        if not recursed:
            candidates.sort()
            candidates.reverse()
            ...
            if num > target:
                ...
                if len(remainder) >= 1:
                    ...
                    while child_target > 0:
                        child_combis = self.combinationSum(remainder, child_target, recursed=True)
                        child_target -= num
                        num_multiple += 1
